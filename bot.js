...
async function startBot() {
  try {
    ...
    console.log(
      "Discord Bot Token (first 5 chars):",
      process.env.DISCORD_BOT_TOKEN ? process.env.DISCORD_BOT_TOKEN.substring(0, 5) + "..." : "undefined",
    )
    console.log("Discord Client ID:", process.env.DISCORD_CLIENT_ID || "undefined")
    console.log(
      "OpenAI API Key (first 5 chars):",
      process.env.OPENAI_API_KEY ? process.env.OPENAI_API_KEY.substring(0, 5) + "..." : "undefined",
    )
    ...
    const commands = [
      new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName("message").setDescription("yap smth random").setRequired(true)),
      ...
      new SlashCommandBuilder().setName("fortune").setDescription("my nigga you're already cooked"),
      ...
      new SlashCommandBuilder().setName("flip").setDescription("flip da coin"),
      ...
      new SlashCommandBuilder().setName("coolshi").setDescription("random cool vids I fw"),
    ]
    ...
    function getRandomCasualResponse() {
      const casualResponses = [
        "no",
        "bro can you like not talk to me",
        "nahhhhh",
        "oh hell nah",
        "aight bro",
        "sybau",
    ...
    function getMagic8BallResponse() {
      const responses = [
        ...
        "Yeah Idk blud",
        "ask again later",
        "I don't get paid enough to answer your shi",
        "cannot predict now.",
        "yeah Im kinda tired now nigga ask later",
        "Don't count on it",
        "I don't see the vision twin",
        "My reply is no.",
        "my sources say no",
        "outlook not so good",
        "highly doubtful.",
    ...
    client.on(Events.InteractionCreate, async (interaction) => {
      ...
      try {
        ...
        switch (commandName) {
          case "yapp": // Fixed: Changed from "chat" to "yapp" to match the command name
          ...
          case "ballsofwisdome": // Fixed: Changed from "eightball" to "ballsofwisdome"
